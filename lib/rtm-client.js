...
var SUBSCRIPTION_SYNC_CHANNEL = '$RMQ/sync/my/subscriptions';
...
function RtmClient(authToken, options) {
  ...
  this.enableSubscriptionSyncEvents = options.enableSubscriptionSyncEvents;
...
RtmClient.prototype = {
  ...
  initSubscriptionSyncEvents: function() {
    var me = this;

    this.subscribe({channel: SUBSCRIPTION_SYNC_CHANNEL}).then(function() {
      me.on(SUBSCRIPTION_SYNC_CHANNEL + '/message', function(message) {
        if (!message || !message.data || !message.data.event) return;
        me.emit(message.data.event, message.data.data);
      });
    });
  }
};
...
function _registerPahoClientEventListeners(rtmClient, pahoClient) {
  ...
  pahoClient.onMessageDelivered = function(pahoMessage) {
    var message = _getSdkMessage(pahoMessage);
...
function _getSdkMessage(pahoMessage) {
  var data;
  var decoded = false;
  var error = null;
  ...
  function decode() {
    try {
      data = JSON.parse(pahoMessage.payloadString);
    } catch (err) {
      error = err;
    }
  }
